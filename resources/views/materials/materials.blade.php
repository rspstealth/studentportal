            ...
            <div id="right-content" class="col-md-10">
                <div class="row justify-content-center">
                    <div class="col-md-12">
                        ...
                        <h3 class="">Materials @can('create-materials',\Illuminate\Support\Facades\Auth::user())<a style="float:right;" href="#" id="new_task_btn" title="Add New Material" class="btn btn-primary "><i class="fas fa-plus"></i> New Material</a>@endcan</h3>
                    ...
                    <div class="col-md-12">
                        <div id="filters_action">
                            <form class="row" id="materials_filters_action_form" style="height:76px;margin: 0;" action="{{url('/materials/list')}}/{{$page_var}}" method="GET">
                                {{ csrf_field() }}
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select id="accessibility_filter" name="accessibility_filter" class="form-control">
                                        <option <?php if(Request::input('single_course_search') == ''){ echo "selected=''";}?> value="all">Generic Materials</option>
...
{{--                                <div class="col-md-2">--}}
{{--                                    Order By--}}
{{--                                    <select id="material_filter" name="material_filter" class="form-control">--}}
...
{{--                                </div>--}}
                    ...
                    <form id="edit_course_form" action="{{url('/materials/')}}/{{$page_var}}" method="POST">
                        {{ csrf_field() }}
                            <div class="col-md-12">
                                <div id="actions_bar">
                                    <div id="actions_bar_left">
                                        ...
                                        @can('delete-materials',\Illuminate\Support\Facades\Auth::user())
                                        <a class="btn btn-outline-secondary tiny-action-btn" href="#" id="multiple_delete_select" title="Delete Selected Materials"><i class="far fa-trash-alt"></i> Delete Selected</a>
                                        @endcan
                                ...
                                <table class="table materials_table">
                                    <thead>
                                    <tr>
                                        ...
                                        <th class="material_description_col" scope="col">Description</th>
                                        <th class="material_file_col" scope="col">Material File</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php
                                    ...
                                    @foreach($materials as $material)
                                        <tr class="record_row record_{{ $material->id }}">
                                            <td class="checkbox_container checkbox_col">
                                                <input class="task_checkbox checkbox" id="{{ $material->id }}" value="{{ $material->id }}" name="delete_checkbox" type="checkbox"/>
                                                <span class="checkmark"></span>
                                            </td>
                                            <td>{{ ($material->course_specific === 'all')? 'Generic Material' : \App\Http\Controllers\CourseController::getCourseNameById($material->course_specific)  }}</td>
                                            <td>{{ $material->description }}</td>
                                            <td><a href="{{ url('/public').'/materials/'.$material->material_file }}" title="Download Material"><i class="fas fa-download"></i> Download</a></td>
                                        </tr>
                                        <?php $count++;?>
                                    @endforeach
                                    <?php
                                    }else{
                                        echo '<tr><td colspan="7"> No Material Found </td></tr>';
                                    }
                                    ?>
                                    </tbody>
                                </table>
                        ...
                        <div class="delete-modal-dialog modal_delete_dialog" role="document">
                            <div class="modal-content align-items-center">
                                <div class="modal-body">
                                    <h3>Confirm Delete Material? <a href="#" title="close dialog" id="close_delete_dialog" style="float: right;margin: 0 10px;"><i class="fa fa-close"></i></a></h3>
                                    <p>This action cannot be undone.</p>
                                </div>
                                <div class="modal-footer">
                                    <form id="del_selected_form" action="{{url('/materials/list')}}/{{$page_var}}" method="POST">
                                        {{ method_field('delete') }}
                                        {{ csrf_field() }}
                    ...
                    <div id="modal_task_action" class="modal modal_task_action" tabindex="-1" role="dialog">
                        <div class="task-modal-dialog modal_task_dialog" style="    width: 50%;    left: 25%;" role="document">
                            <div class="modal-content align-items-center" style="padding: 0 20px;">
                                <div class="modal-body">
                                    <h3>Add Material <a href="#" title="close dialog" id="close_dialog" style="float: right;margin: 0 10px;"><i class="fa fa-close"></i></a></h3>
                                </div>
                                <div class="modal-footer">
                                    <form id="add_material_form" action="{{url('/materials/list/')}}/{{$page_var}}" method="POST" enctype="multipart/form-data">
                                        {{ csrf_field() }}
                                        <div class="col-md-12 no-padding">
                                            <div class="col-md-12 no-padding">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label><input checked="" type="radio" class="course_specific" name="course_specific" value="all">Generic Material (Available for all courses)</label>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        Material File
                                                        <input required="" type="file" class="form-control lb-lg"  id="material_file" name="material_file" />
                                                        <small class="text-danger error_msg" id="material_file_errors"></small>
                                                    </div>
                                                </div>
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                        ...
                                        <input type="hidden" id="total" name="total" value="{{$total}}" />
                                        <input type="hidden" id="totalPages" name="totalPages" value="{{$totalPages}}" />
                                        <input type="hidden" id="method" name="method" value="new_material" />
                                        <input type="hidden" id="course_id" name="course_id" value="" />
                                        <input type="hidden" id="page_var" name="page_var" value="{{$page_var}}" />
                                        <div class="col-md-12">
                                            <hr/>
                                            <input class="btn btn-primary" type="submit" id="new_material_btn" name="new_material_btn" value="Add Material" />
                                            <button type="button" id="cancelled_new_material_btn" class="btn btn-outline-secondary ">Cancel</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
...
<script>
    (function(){
        ...
        $("#cancelled_new_material_btn").on("click", function (e) {
        ...
        $(".selector_checkbox").on("click", function () {
            if($(".selector_checkbox").prop('checked') == true){
                ...
                $('.selected_label').text(countCheckedCheckboxes + ' Material(s) Selected');
        ...
        //add material
        $("#new_material_btn").click(function (e) {
            var error_scroll = '';
            if ($('#material_file').val().length <= 0) {
                $('#material_file_errors').text('Please select a material file to upload');
                return false;
            } else {
                $('#material_file_errors').text('');
            }
            ...
            $.ajax({
                url: '{{ url('/materials/list/').'/'.$page_var }}',
                type: 'POST',
                data: formdata,
                processData: false,
                contentType: false,
                success: function (response) {
                    $('#success-alert').html('<h3>Material Created Successfully</h3>');
                    $("#new_material_btn").attr('disabled',true);
                    $('#success-alert').focus();
                    $("#success-alert").fadeTo(2000, 500).slideUp(500, function () {
                        $("#success-alert").css("height", "0px");
                        $("#success-alert").css("display", "block");
                        var page = '<?php echo url("/")?>'+'/materials/list/<?php echo $page_var;?>';
